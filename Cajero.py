...
Jorgito = Cliente(100, "C001", "pass1", [])
Cathy = Cliente(200, "C002", "pass2", [])

file = open("sueldos.txt", "r")
lista_de_sueldos = file.readlines()
file.close()
print(lista_de_sueldos)
saldos = []
for su in lista_de_sueldos:
    reversed_lista = su[::-1]
    reversed_sueldo = reversed_lista[1:]
    sueldo_como_tal = reversed_sueldo[::-1]
    saldos.append(int(sueldo_como_tal))
print(saldos)

file2 = open("contraseñas.txt", "r")
lista_de_contrasenas = file2.readlines()
file2.close()
print(lista_de_contrasenas)
passwords = []
for co in lista_de_contrasenas:
    reversed_lista2 = co[::-1]
    reversed_contrasena = reversed_lista2[1:]
    contrasena_como_tal = reversed_contrasena[::-1]
    passwords.append(contrasena_como_tal)
print(passwords)

file3 = open("movimientos.txt", "r")
lista_de_movimientos = file3.readlines()
file3.close()
...
codigos = [Jorgito.codigo, Cathy.codigo]
# passwords = [Jorgito.password, Cathy.password]
# saldos = [Jorgito.saldo, Cathy.saldo]
# movimientos = [Jorgito.movimiento, Cathy.movimiento]
...
def movements():
    ...
    tupla_movimientos_indice = tuple(movimientos[indice])
    cadena = "\n".join(tupla_movimientos_indice)
...
def retirar(n):
    ...
    if n <= saldos[indice]:
        ...
        file = open("sueldos.txt", "w")
        saldos[indice] -= n
        cadena_saldos = ""
        for sal in saldos:
            cadena_saldos += str(sal) + "\n"
        file.write(cadena_saldos)
        file.close()
        ...
        movimiento = f"Retiro\t\t\t     -{n}"
        movimientos[indice].insert(0, movimiento)
        print(movimientos[indice])
...
def transferir(n, a):
    ...
    if a in codigos and n <= saldos[indice]:
        ...
        movimiento = f"Transferencia\t\t\t     -{n}"
        movimientos[indice].insert(0, movimiento)
        print(movimientos[indice])

        movimiento2 = f"Transferencia\t\t\t     +{n}"
        movimientos[indice2].insert(0, movimiento2)
...
def cambioClave(a, b, c):
    ...
    if a == passwords[indice]:
        if len(b) > 0 and b == c:
            ...
            cadena_contrasenas = ""
            for cont in passwords:
                cadena_contrasenas += cont + "\n"
            file2.write(cadena_contrasenas)
            file2.close()
